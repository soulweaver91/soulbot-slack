        ...
        try:
            if r.status_code == 200:
                ...
                if "It's just you" in status or "If you can see this page and still think we're down" in status:
                    return outputs.append([data["channel"],
            ...
            return outputs.append([data["channel"], 'ERROR: Check failed :cry: Please try again later!'])
    else:
        return outputs.append([data["channel"], 'Please tell me the site to check first.'])


def process_message(data):
    if data["soulbot_command"] in ['isup', 'isdown', 'down']:
        return isup_site(data)
